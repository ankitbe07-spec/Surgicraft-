...
DEF_SETTINGS = {
    ...
    "hsn_codes": [],
    "vis_mach": ['Date', 'Item Details', 'Final Price'], # REMOVED OLD DATE/PRICE FROM DEFAULTS, RENAMED FINAL PRICE
    "vis_part": ['Date', 'HSN Code', 'Final Price']  # RENAMED FINAL PRICE
}
...
def load_settings_from_sheet():
    try:
        ...
        if val:
            ...
            # Smart check to replace old price column name in saved settings
...
# --- Item Description (Screen/PDF - NO Machine/Part label) ---
...
# --- PROCESSED DATAFRAME FOR VIEWING ---
def prepare_display_df_with_history(df):
    ...
    df['Note'] = notes # Extracted note for screen view
...
# --- PDF with Fixed Sequential Sr. No. & Centered Prices ---
def create_dynamic_pdf(party, records_df, title_str, visible_cols, is_machine=True):
    buffer = io.BytesIO()
    c = canvas.Canvas(buffer, pagesize=landscape(A4))
    width, height = landscape(A4)
    
    c.setFont("Helvetica-Bold", 14); c.drawString(40, height - 40, title_str)
    ...
    # Check if Item Details is visible to calculate stretch
    ...
    if has_item_details:
        ...
        # If no Item Details, scale other visible columns proportionally to fill page width
        if fixed_sum > 0:
            scale_factor = avail_width / fixed_sum
            for col in vis_pdf_cols: col_widths[col] = col_widths[col] * scale_factor
        final_avail = avail_width
        
    cols = [start_x]
    for col in vis_pdf_cols: cols.append(cols[-1] + col_widths.get(col, 80))
    ...
    for i, col in enumerate(vis_pdf_cols):
        ...
        else: c.drawCentredString(mid_x, y+2, col)
    ...
    for index, row in records_df.iterrows():
        ...
        # Determine extra space needed for Note & Add-ons
        ...
        max_drop = 10 # Base drop for standard text
        
        # Draw standard dynamic columns
        for i, col in enumerate(vis_pdf_cols):
            mid_x = (cols[i]+cols[i+1])/2.0
            if col == 'Sr. No.': c.drawCentredString(mid_x, text_y, str(enum_counter))
            elif col == 'Date': c.drawCentredString(mid_x, text_y, dt_val)
            ...
            elif col == 'Final Price': c.drawCentredString(mid_x, text_y, new_price_str) # RENAMED/CENTERED
                ...
                # Draw Add-ons indented
                if is_machine and not opts.get('Is_Custom_Name', False):
                    c.setFont("Helvetica-Oblique", 8)
                    for name in addons:
                        c.drawString(cols[i]+15, temp_y, f"• Add-on: {name}"); temp_y -= 15
                        
                # Update drop based on details section
                max_drop = max(max_drop, (text_y - temp_y))
                c.setFont("Helvetica-Bold", 9) # Reset for other cols in row
                
        grid_y_bot = text_y - max_drop - 5
        draw_grid_lines(c, row_y_top, grid_y_bot, cols)
        
        # --- DRAW NOTE BELOW GRID IF EXISTS ---
        current_y = grid_y_bot
        if note_str:
            c.setFont("Helvetica-Oblique", 9)
            # Find Item Details column index for note indent
            ...
            current_y -= 18 # Extra padding below note for next row top line

        y = current_y # Update y for next row top line
        enum_counter += 1
...
def create_factory_pdf(raw_material, search_part, df, orientation="Aadu (Landscape)"):
    ...
    if "Portrait" in orientation:
        pagesize_selected = A4; cols = [30, 85, 185, 310, 385, 450, 490, 565] 
    else:
        pagesize_selected = landscape(A4); cols = [40, 110, 290, 450, 560, 680, 740, 800] 
        
    width, height = pagesize_selected; c = canvas.Canvas(buffer, pagesize=pagesize_selected)
    c.setFont("Helvetica-Bold", 14); c.drawString(cols[0], height - 40, "Surgicraft Factory Production & Cutting List")
    c.setFont("Helvetica", 10); c.drawString(cols[0], height - 60, f"Material: {raw_material} | Part: {search_part}"); c.drawString(cols[4], height - 60, f"Date: {datetime.now().strftime('%d-%m-%Y')}")
    
    y = height - 100; c.setFont("Helvetica-Bold", 9)
    row_y_top = y + 20; row_y_bot = y - 5
    headers = ["Date", "Raw Material", "Part Name", "Cut Size", "Final Size", "Qty", "Date"]
    for i,h in enumerate(headers): c.drawCentredString((cols[i]+cols[i+1])/2.0, y+2, h)
    draw_grid_lines(c, row_y_top, row_y_bot, cols); y = row_y_bot
    ...
    for index, row in df.iterrows():
        ...
        row_y_top = y; text_y = y - 15; c.setFont("Helvetica", 8)
        ...
        f_sz = str(row.get('Final Size', '')); c.drawCentredString((cols[4]+cols[5])/2.0, text_y, f_sz if f_sz not in ['nan','','-'] else '')
...
def create_hexo_pdf(mat_name, mat_in, mat_out, balance_mm, df):
    ...
    c.setFont("Helvetica", 10); c.drawString(40, 755, f"📥 Total In: {mm_to_foot_inch(mat_in)}")
    c.drawString(40, 740, f"✂️ Total Out: {mm_to_foot_inch(mat_out)}")
    c.setFillColorRGB(0, 0.5, 0); c.setFont("Helvetica-Bold", 11); c.drawString(40, 720, f"✅ Balance: {mm_to_foot_inch(balance_mm)} ({balance_mm:.1f} MM)"); c.setFillColorRGB(0, 0, 0)
    
    y = 690; c.setFont("Helvetica-Bold", 10); cols = [40, 105, 195, 255, 345, 550]
    row_y_top = y + 20; row_y_bot = y - 5
    hs = ["Date", "Cut Size", "Qty", "Blade Margin", "Total Used (MM)"]
    for i,h in enumerate(hs): c.drawCentredString((cols[i]+cols[i+1])/2.0, y+2, h)
    draw_grid_lines(c, row_y_top, row_y_bot, cols); y = row_y_bot
    
    for index, row in df.iterrows():
        if y - 25 < 50:
            c.showPage(); y = 800; c.setFont("Helvetica-Bold", 10); row_y_top = y+20; row_y_bot = y-5
            for i,h in enumerate(hs): c.drawCentredString((cols[i]+cols[i+1])/2.0, y+2, h)
            draw_grid_lines(c, row_y_top, row_y_bot, cols); y = row_y_bot
            
        row_y_top = y; text_y = y - 15; c.setFont("Helvetica", 9)
        c.drawCentredString((cols[0]+cols[1])/2.0, text_y, str(row['Date'])[:10])
        ...
        c.setFont("Helvetica-Bold", 10); c.drawRightString(cols[5]-10, text_y, f"{float(row['Total Used (MM)']):.1f}")
        row_y_bot = text_y - 5; draw_grid_lines(c, row_y_top, row_y_bot, cols); y = row_y_bot
...
if menu == "🪚 Hexo Cutting (Live Stock)":
    ...
    with htab1:
        ...
        sc1, sc2 = st.columns(2); cut_size_str = sc1.text_input("Size:", key="cut_size_hexo"); cut_unit = sc2.selectbox("Unit:", ["MM", "Inch", "Foot"], key="unit_hexo")
        c3, c4 = st.columns(2); cut_qty = c3.number_input("Qty (Nang):", min_value=1, key="qty_hexo"); blade_margin = c4.number_input("Margin:", value=1.5, step=0.1, key="margin_hexo")
        ...
        if cut_size_str and cut_mat and cut_mat != "-- Select --":
            ...
            if size_val > 0 and cut_qty > 0:
                size_in_mm = convert_to_mm(size_val, cut_unit); total_used_mm = (size_in_mm + blade_margin) * cut_qty
                ...
                cur_bal = cur_in - cur_out; new_bal = cur_bal - total_used_mm
                st.info(f"👉 **Jarur:** {mm_to_foot_inch(total_used_mm)} total maal."); st.info(f"👉 **Balance:** {mm_to_foot_inch(cur_bal)} -> **{mm_to_foot_inch(new_bal)}** vadhse.")
                if rod_foot > 0:
                    rod_mm = rod_foot * 304.8; rods_needed = math.ceil(total_used_mm / rod_mm); wastage = (rods_needed * rod_mm) - total_used_mm
                ...
                if st.button("✂️ Kapi Nakho", type="primary", key="btn_save_hexo"):
                    ...
                    if mat_sel == "-- New --" and cut_mat not in stock_materials_full: sheet_stock.append_row([dt_str, cut_mat.strip(), 0, 0, 0])
                    sheet_hexo.append_row([dt_str, cut_mat.strip(), f'{cut_size_str} {cut_unit}', cut_qty, blade_margin, total_used_mm]); st.success("Cutting saved!"); clear_all_caches(); st.rerun()
    with htab2:
        new_mat_name = st.text_input("Raw Material Name:", key="new_stock_name")
        col_v, col_u, col_k = st.columns(3); in_val_str = col_v.text_input("Length:", key="new_stock_len"); in_unit = col_u.selectbox("Unit:", ["Foot", "Inch", "MM"], key="new_stock_unit"); weight_kg = col_k.number_input("Weight (KG):", key="new_stock_weight")
        if st.button("💾 Save Stock", type="primary", key="btn_save_stock"):
            in_val = parse_smart_size(in_val_str) if in_val_str else 0
            if not new_mat_name or in_val <= 0: st.warning("Enter name and length!")
            else:
                total_mm = convert_to_mm(in_val, in_unit); total_foot = total_mm / 304.8
                sheet_stock.append_row([datetime.now().strftime("%d-%m-%Y"), new_mat_name.strip(), total_foot, total_mm, weight_kg]); st.toast("Saved!"); clear_all_caches(); st.rerun()
    with htab3:
        ...
        if not stock_df.empty:
            ...
            for mat in filtered_mats:
                ...
                with st.expander(f"📦 {mat} | Bal: {mm_to_foot_inch(bal_mm)}", expanded=(len(filtered_mats)==1)):
                    sc1, sc2, sc3 = st.columns(3); sc1.metric("Total In", mm_to_foot_inch(mat_in)); sc2.metric("Total Out", mm_to_foot_inch(mat_out)); sc3.metric("Live Balance", mm_to_foot_inch(bal_mm))
                    if not mat_hexo_df.empty:
                        ...
                        pdf_buf = create_hexo_pdf(mat, mat_in, mat_out, bal_mm, mat_hexo_df); c_dl, c_pv = st.columns(2)
                        with c_dl: st.download_button("📥 PDF", data=pdf_buf, file_name=f"{mat}_Report.pdf", use_container_width=True, key=f"dl_{mat}")
                        with c_pv: 
                            if st.button(f"👁️ Preview", key=f"pv_{mat}", use_container_width=True): display_pdf_in_app(pdf_buf)
    with htab4:
        edit_type = st.radio("Edit?", ["Cutting", "Stock"], horizontal=True, key="edit_type_radio")
        if edit_type == "Cutting":
            if hexo_df.empty: st.info("No records.")
            else:
                h_df = hexo_df.copy(); h_df['Display'] = h_df['Date'].astype(str) + " | " + h_df['Material Name'].astype(str) + " | Size: " + h_df['Cut Size'].astype(str)
                sel_h = st.selectbox("Select Cutting:", h_df['Display'].tolist(), key="edit_hexo_sel")
                if sel_h:
                    r_d = h_df[h_df['Display'] == sel_h].iloc[0]; ks = str(hash(sel_h)); e1, e2 = st.columns(2); c_mat_i = stock_materials_full.index(str(r_d['Material Name'])) if str(r_d['Material Name']) in stock_materials_full else 0
                    n_mat = e1.selectbox("Edit Material:", stock_materials_full, index=c_mat_i, key=f"eh_mat_{ks}")
                    o_sz = str(r_d['Cut Size']); o_ut = "MM"; o_val_s = o_sz.strip()
                    if "Inch" in o_sz: o_ut = "Inch"; o_val_s = o_sz.replace('Inch','').strip()
                    elif "Foot" in o_sz: o_ut = "Foot"; o_val_s = o_sz.replace('Foot','').strip()
                    elif "MM" in o_sz: o_ut = "MM"; o_val_s = o_sz.replace('MM','').strip()
                    es1, es2 = st.columns(2); n_cut = es1.text_input("Edit Size:", value=o_val_s, key=f"eh_size_{ks}"); n_unit = es2.selectbox("Unit:", ["MM", "Inch", "Foot"], index=["MM", "Inch", "Foot"].index(o_ut), key=f"eh_unit_{ks}")
                    e3, e4 = st.columns(2); n_qty = e3.number_input("Qty:", value=safe_int(r_d['Quantity'], 1), key=f"eh_qty_{ks}"); n_margin = e4.number_input("Margin:", value=safe_float(r_d['Blade Margin (MM)'], 1.5), key=f"eh_margin_{ks}")
                    b1, b2 = st.columns(2)
                    if b1.button("💾 Update", key=f"btn_upd_h_{ks}"):
                        n_val = parse_smart_size(n_cut)
                        if n_val > 0:
                            n_mm = convert_to_mm(n_val, n_unit); n_total = (n_mm + n_margin) * n_qty
                            for i, r in enumerate(sheet_hexo.get_all_values()):
                                if i > 0 and r[0] == str(r_d['Date']) and r[1] == str(r_d['Material Name']) and str(r[2]) == str(r_d['Cut Size']):
                                    sheet_hexo.update(f"B{i+1}:F{i+1}", [[n_mat, f"{n_cut} {n_unit}", n_qty, n_margin, n_total]]); st.success("Updated!"); clear_all_caches(); st.rerun(); break
                    if b2.button("❌ Delete", key=f"btn_del_h_{ks}"):
                        for i, r in enumerate(sheet_hexo.get_all_values()):
                            if i > 0 and r[0] == str(r_d['Date']) and r[1] == str(r_d['Material Name']) and str(r[2]) == str(r_d['Cut Size']):
                                sheet_hexo.delete_rows(i+1); st.success("Deleted!"); clear_all_caches(); st.rerun(); break
        else:
            if stock_df.empty: st.info("No records.")
            else:
                s_df = stock_df.copy(); s_df['Display'] = s_df['Date'].astype(str) + " | " + s_df['Material Name'].astype(str)
                sel_s = st.selectbox("Select Stock:", s_df['Display'].tolist(), key="edit_stock_sel")
                if sel_s:
                    r_d = s_df[s_df['Display'] == sel_s].iloc[0]; ks = str(hash(sel_s)); e1, e2 = st.columns(2); n_mat = e1.text_input("Name:", value=str(r_d['Material Name']), key=f"es_mat_{ks}"); n_wt = e2.number_input("Weight:", value=safe_float(r_d.get('Weight (KG)', 0.0)), key=f"es_wt_{ks}")
                    es1, es2 = st.columns(2); n_len = es1.text_input("Add Length (Opt):", key=f"es_len_{ks}"); n_unit = es2.selectbox("Unit:", ["Foot", "Inch", "MM"], key=f"es_unit_{ks}")
                    b1, b2 = st.columns(2)
                    if b1.button("💾 Update", key=f"btn_upd_s_{ks}"):
                        ...
                        if n_len:
                            ...
                            if n_val > 0: n_total_mm = convert_to_mm(n_val, n_unit); n_total_ft = n_total_mm / 304.8
                        for i, r in enumerate(sheet_stock.get_all_values()):
                            if i > 0 and r[0] == str(r_d['Date']) and r[1] == str(r_d['Material Name']):
                                sheet_stock.update(f"B{i+1}:E{i+1}", [[n_mat, n_total_ft, n_total_mm, n_wt]]); st.success("Updated!"); clear_all_caches(); st.rerun(); break
...
# ==========================================
    ...
    with tabA:
        c01, c02 = st.columns(2); rec_date = c01.date_input("Date:", datetime.today(), key="fac_date")
        c1, c2 = st.columns(2); raw_sel = c1.selectbox("Material:", ["-- Empty --", "-- New --"] + unique_materials, key="fac_raw_sel"); raw_val = c1.text_input("New Name:", key="fac_new_raw") if raw_sel == "-- New --" else raw_sel
        part_sel = c2.selectbox("Part Name:", ["-- New --"] + unique_factory_parts, key="fac_part_sel"); part_val = c2.text_input("New Part:", key="fac_new_part") if part_sel == "-- New --" else part_sel
        c3, c4, c5 = st.columns([1.5, 1.5, 1]); cut_size = c3.text_input("Cut Size", key="fac_cut_sz"); final_size = c4.text_input("Final Size", key="fac_fin_sz"); qty = c5.number_input("Qty", min_value=1, key="fac_qty")
        if st.button("💾 Save", type="primary", key="btn_save_fac"):
            if not part_val or part_val == "-- New --" or not cut_size: st.warning("Name & Cut Size compulsory!"); st.stop()
            sheet_factory.append_row([rec_date.strftime("%d-%m-%Y"), raw_val if raw_val != "-- Empty --" else "-", part_val.strip(), cut_size.strip(), final_size.strip() if final_size else "-", int(qty)]); st.toast("Saved!"); clear_all_caches(); st.rerun()
    with tabB:
        skw = st.text_input("🔍 Search:", key="search_fac"); c1, c2 = st.columns(2)
        search_raw = c1.selectbox("Filter Material:", ["-- All --"] + unique_materials, key="search_fac_raw"); search_part = c2.selectbox("Filter Part:", ["-- All --"] + unique_factory_parts, key="search_fac_part")
        f_df = factory_df.copy()
        if not f_df.empty:
            if search_raw != "-- All --": f_df = f_df[f_df['Raw Material'].astype(str).str.strip() == search_raw]
            if search_part != "-- All --": f_df = f_df[f_df['Part Name'].astype(str).str.strip() == search_part]
            if skw: f_df = f_df[f_df[['Raw Material', 'Part Name', 'Cutting Size']].astype(str).apply(lambda x: x.str.contains(skw, case=False)).any(axis=1)]
            st.dataframe(f_df, use_container_width=True, hide_index=True)
            tqty = pd.to_numeric(f_df['Quantity'], errors='coerce').fillna(0).sum(); st.success(f"**Total Qty: {int(tqty)}**")
            st.write("---"); pdf_fmt = st.radio("PDF Format:", ["Landscape", "Portrait"], horizontal=True, key="fac_pdf_format")
            f_pdf = create_factory_pdf(search_raw, search_part, f_df, pdf_fmt); c1, c2 = st.columns(2)
            with c1: st.download_button("📥 PDF", data=f_pdf, file_name="Factory_List.pdf", use_container_width=True, key="dl_fac_pdf")
            with c2: 
                if st.button("👁️ Preview", use_container_width=True, key="pv_fac_pdf"): display_pdf_in_app(f_pdf)
    with tabC:
        if factory_df.empty: st.info("No records.")
        else:
            e_df = factory_df.copy(); e_df['Final Size'] = e_df.get('Final Size', ''); e_df['Display'] = e_df['Date'].astype(str) + " | " + e_df['Part Name'].astype(str) + " | " + e_df['Cutting Size'].astype(str)
            sel_r = st.selectbox("Select Record:", e_df['Display'].tolist(), key="edit_fac_sel")
            if sel_r:
                r_d = e_df[e_df['Display'] == sel_r].iloc[0]; ks = str(hash(sel_r)); e_d = st.date_input("Date:", safe_date(str(r_d['Date'])), key=f"ef_date_{ks}")
                e1, e2 = st.columns(2); n_raw = e1.text_input("Material:", value=str(r_d['Raw Material']), key=f"ef_raw_{ks}"); n_prt = e2.text_input("Part:", value=str(r_d['Part Name']), key=f"ef_part_{ks}")
...
# ==========================================
    ...
    if entry_type == "Machine":
        ...
        st.write("### Add-ons"); cls = st.columns(3); sel_ads, ads_struct, ci = [], {}, 0
        lhl = settings.get('lh_label', 'Low+High Speed Extra Charge')
        if speed == "Low+High": ads_struct[lhl] = settings['addons'].get(lhl, 0)
        if cust_dtl.strip(): ads_struct["Custom_Details"] = cust_dtl.strip()
        for an in settings['addons']:
            if an == lhl: continue
            if cls[ci%3].checkbox(an, key=f"chk_{an}"): ads_struct[an] = settings['addons'][an]
            ci += 1
        ...
        st.info(f"💡 Idea Price: Rs. {calc_t:,.2f}"); final_t = st.number_input("Final Machine Price:", value=calc_t, key="btn_add_manual_price")
        
        # --- GENERAL NOTE FOR MACHINE ---
        gen_note = st.text_area("🗒️ Machine Note / Remarks (Optional - Visible on PDF):", key="add_mach_note")
        if gen_note.strip(): ads_struct["General_Note"] = gen_note.strip()
        
        if st.button("➕ SAVE MACHINE", type="primary", key="btn_add_entry"):
            if not party_name: st.warning("Party Name compulsory!")
            else:
                sheet_main.append_row([st.session_state.q_no, party_name.strip().title(), datetime.now().strftime("%d-%m-%Y"), f"{w_val}x{l_val}", speed if speed!="-- None --" else "-", json.dumps(ads_struct), final_t]); st.toast("Saved!"); clear_all_caches(); st.rerun()

    else:
        st.write("### Spare Part Details"); c1, c2 = st.columns(2)
        ...
        hsns = c3.selectbox("HSN Code:", ["-- New --"] + hsnl, key="add_sp_hsn_sel"); hsn_v = c3.text_input("📝 New HSN:", key="add_sp_hsn_new") if hsns == "-- New --" else hsns
        gst_r = c4.selectbox("GST (%)", [0] + sorted(settings.get("gst_rates", [])), key="add_sp_gst")
        final_c = basic_p + (basic_p * gst_r / 100); st.info(f"**Final: Rs. {final_c:,.2f}**")
        
        # --- GENERAL NOTE FOR SPARE PART ---
        gen_note_sp = st.text_area("🗒️ Spare Part Note / Remarks (Optional - Visible on PDF):", key="add_sp_note")
        opts_sp = {"Basic": basic_p, "GST": gst_r, "HSN": hsn_v if hsn_v!="None" else "-"}
        if gen_note_sp.strip(): opts_sp["General_Note"] = gen_note_sp.strip()
        
        if st.button("➕ SAVE PART", type="primary", key="btn_add_part"):
            if not party_name or not p_name or p_name=="-- New --": st.warning("Enter Name!"); st.stop()
            sheet_main.append_row([st.session_state.q_no, party_name.strip().title(), datetime.now().strftime("%d-%m-%Y"), p_name.strip(), "Spare Part", json.dumps(opts_sp), final_c]); st.toast("Saved!"); clear_all_caches(); st.rerun()
    ...
    if main_df.empty: st.info("No records.")
        ...
        with tab1:
            ...
            if pdf_p != "-- Select --":
                ...
                if skw: p_df = p_df[p_df.astype(str).apply(lambda x: x.str.contains(skw, case=False)).any(axis=1)]
                
                if p_df.empty: st.warning("No matches.")
                else:
                    ...
                    if set(sel_mach) != set(saved_mach) or set(sel_part) != set(saved_part):
                        settings['vis_mach'] = sel_mach; settings['vis_part'] = sel_part; save_settings_to_sheet(settings); st.toast("Saved! ✅")
                    
                    st.write("---"); col1, col2 = st.columns(2)
                    ...
                    with col2:
                        ...
                        else: st.info("No parts.")

        with tab2:
            edit_p = st.selectbox("Select Party (Edit):", ["-- Select --"] + unique_parties_list, key="edit_hist_party")
            if edit_p != "-- Select --":
                p_items = prepare_display_df_with_history(df[df['Clean_Party'] == edit_p]); p_items['Display'] = p_items.apply(make_full_display_name, axis=1)
                ...
                if sel_i:
                    ...
                    if is_sp:
                        ...
                        n_basic = st.number_input("Basic:", value=safe_int(o_basic, 0), key=f"eh_sp_{ks}"); c1, c2 = st.columns(2)
                        hsnl = ["None"] + sorted(settings.get("hsn_codes", [])); if o_hsn and o_hsn not in hsnl and o_hsn!="-": hsnl.append(o_hsn)
                        hsns = c1.selectbox("Edit HSN:", ["-- New --"] + hsnl, index=hsnl.index(o_hsn)+1 if o_hsn in hsnl else 0, key=f"eh_hsns_{ks}"); n_hsn = c1.text_input("New HSN:", value=o_hsn if hsns=="-- New --" else "", key=f"eh_hsnn_{ks}") if hsns=="-- New --" else hsns
                        n_gst = c2.selectbox("GST:", [0] + sorted(settings.get("gst_rates", [])), index=sorted(settings.get("gst_rates", [0])).index(o_gst) if o_gst in settings.get("gst_rates",[]) else 0, key=f"eh_gst_{ks}")
                        n_price = d4.number_input("Final Price:", value=int(n_basic + (n_basic*n_gst/100)), key=f"eh_spf_{ks}")
                    else: n_price = d4.number_input("Final Price:", value=safe_int(r_d['Total_Price'], 0), key=f"eh_mf_{ks}")
        ...
        with tab4:
            ...
            if cf != "-- Select --":
                p_data = prepare_display_df_with_history(df[df['Clean_Party'] == cf]); p_data['Display'] = p_data.apply(make_full_display_name, axis=1)
                sel_cls = []; st.write("**Select Items:**")
                for i, d in enumerate(p_data['Display'].tolist()):
                    if st.checkbox(d, value=True, key=f"cl_{cf}_{i}"): sel_cls.append(d)
                st.write("---"); c1, c2 = st.columns(2); pct = c1.number_input("% Change (+/-):", key="clone_pct"); npt = c2.text_input("New Party Name:", key="clone_new_party")
                if st.button("🚀 Clone", type="primary", key="btn_clone_party"):
                    ...
                    for d in sel_cls:
                        ...
                        if is_sp:
                            ...
                            try:
                                opts_d = json.loads(n_opt); lhl = settings.get('lh_label','Low+High Speed Extra Charge')
                                for k,v in list(opts_d.items()):
                                    if k not in ['HSN', 'ManualOldDate', 'ManualOldPrice', lhl, 'Custom_Details', 'Is_Custom_Name', 'General_Note'] and isinstance(v, (int, float)): opts_d[k] = int(v * (1 + (pct/100.0)))
                                opts_d['ManualOldDate'] = "-"; opts_d['ManualOldPrice'] = "-"; n_opt = json.dumps(opts_d)
                            except: pass
                        n_rows.append([st.session_state.q_no, npt.strip().title(), dt_s, r_d['Size'], str(r_d['Speed']), n_opt, n_ttl])
                    sheet_main.append_rows(n_rows); st.success(f"Cloned {len(n_rows)} to '{npt}'! ✅"); clear_all_caches(); st.rerun()
...
# ==========================================
    ...
    if main_df.empty: st.info("No records.")
        ...
        df = main_df.copy(); df['Clean_Party'] = df['Party'].astype(str).str.strip().str.title()
        skw = st.text_input("🔍 Smart Search (Name/Size):", key="search_pf"); c1, c2 = st.columns(2)
        sp = c1.selectbox("Filter Party:", ["-- All --"] + unique_parties_list, key="search_pf_party"); f_items = sorted(df[df['Clean_Party'] == sp]['Size'].astype(str).unique().tolist()) if sp != "-- All --" else all_items_list
        si = c2.selectbox("Filter Item:", ["-- All --"] + f_items, key="search_pf_item")
        f_df = df.copy()
        if sp != "-- All --": f_df = f_df[f_df['Clean_Party'] == sp]
        if si != "-- All --": f_df = f_df[f_df['Size'].astype(str).str.strip() == si]
        if skw: f_df = f_df[f_df[['Size', 'Speed', 'Party']].astype(str).apply(lambda x: x.str.contains(skw, case=False)).any(axis=1)]
        
        if f_df.empty: st.warning("No entries.")
            ...
            with c2: 
                if st.button("👁️ Preview", use_container_width=True, key="pv_pf_pdf"): display_pdf_in_app(p_buf)
...
# ==========================================
    ...
    c1, c2 = st.columns(2); ms = ["01","02","03","04","05","06","07","08","09","10","11","12"]
    cm = datetime.now().strftime("%m"); cy = datetime.now().year; ys = [str(y) for y in range(cy-2, cy+3)]
    sel_m = c1.selectbox("Month:", ms, index=ms.index(cm)); sel_y = c2.selectbox("Year:", ys, index=ys.index(str(cy)))
    tstr = f"-{sel_m}-{sel_y}"; dmstr = f"{datetime.strptime(sel_m, '%m').strftime('%B')} {sel_y}"; st.write("---")
    if st.button("🚀 Send Email Now", type="primary", key="btn_send_mail"):
        with st.spinner(f"Preparing reports for {dmstr}..."):
            pdf_atts = {}
            f_df = factory_df.copy(); if not f_df.empty:
                f_df = f_df[f_df['Date'].astype(str).str.endswith(tstr)]
                if not f_df.empty: pdf_atts[f"Factory_{dmstr}.pdf"] = create_factory_pdf("-- All --", "-- All --", f_df)
            h_df = hexo_df.copy(); if not h_df.empty:
                ...
                if not h_df.empty:
                     ...
                     mat_out = pd.to_numeric(h_df['Total Used (MM)'], errors='coerce').fillna(0).sum(); pdf_atts[f"Hexo_{dmstr}.pdf"] = create_hexo_pdf("All Materials", mat_in, mat_out, mat_in - mat_out, h_df)
            m_df = main_df.copy(); if not m_df.empty:
...
# ==========================================
    ...
    with tab1:
        st.subheader("Base Sizes"); prices = settings['prices']
        for size, price in list(prices.items()):
            cA, cB, cC = st.columns([2, 2, 1]); cA.write(f"**{format_size(size)}**")
            prices[size] = cB.number_input("Price", value=price, step=1000, key=f"p_{size}", label_visibility="collapsed")
            if cC.button("❌", key=f"d_{size}"): del prices[size]; save_settings_to_sheet(settings); st.rerun()
        st.write("---"); c1, c2, c3 = st.columns(3); n_w = c1.text_input("W (e.g. 24)"); n_l = c2.text_input("L (e.g. 48)"); n_p = c3.number_input("Base Price", step=1000)
        if st.button("➕ Add Size", key="btn_set_sz") and n_w and n_l and n_p > 0: settings['prices'][f"{n_w}x{n_l}"] = n_p; save_settings_to_sheet(settings); st.rerun()
    with tab2:
        st.subheader("Add-ons"); addons = settings['addons']
        lhl = settings.get('lh_label', 'Low+High Speed Extra Charge')
        lhp = settings['addons'].get(lhl, 0)
        cA, cB, cC = st.columns([2, 2, 1]); new_lh_l = cA.text_input("Special Label", value=lhl); new_lh_p = cB.number_input("Price", value=lhp, step=500)
        if cC.button("💾 Label", key="btn_ren_lh"):
            if new_lh_l != lhl: settings['addons'][new_lh_l] = new_lh_p; if lhl in settings['addons']: del settings['addons'][lhl]; settings['lh_label'] = new_lh_l
            else: settings['addons'][lhl] = new_lh_p
            save_settings_to_sheet(settings); st.rerun()
        st.write("---")
        for name, price in list(addons.items()):
            if name == settings.get('lh_label'): continue
            cA, cB, cC = st.columns([2, 2, 1]); cA.write(f"**{name}**")
            addons[name] = cB.number_input("Price", value=price, step=500, key=f"a_{name}", label_visibility="collapsed")
            if cC.button("❌", key=f"da_{name}"): del addons[name]; save_settings_to_sheet(settings); st.rerun()
        if st.button("💾 Save Add-ons", type="primary"): save_settings_to_sheet(settings); st.toast("Saved!")
        st.write("---"); c1, c2 = st.columns(2); new_a = c1.text_input("Name:"); new_p = c2.number_input("Price:", step=500)
        if st.button("➕ Add", key="btn_set_add") and new_a and new_p > 0: settings['addons'][new_a] = new_p; save_settings_to_sheet(settings); st.rerun()
    with tab3:
        st.subheader("GST rates"); gst_rates = settings.get("gst_rates", [])
        for g in list(gst_rates):
            cA, cB = st.columns([3, 1]); cA.write(f"**{g}%**")
            if cB.button("❌", key=f"dgst_{g}"): gst_rates.remove(g); settings["gst_rates"] = gst_rates; save_settings_to_sheet(settings); st.rerun()
        st.write("---"); n_gst = st.number_input("New GST %", min_value=1, max_value=100)
    ...
    with tab4:
        st.subheader("HSN Codes"); hsn_codes = settings.get("hsn_codes", [])
        for h in list(hsn_codes):
            cA, cB = st.columns([3, 1]); cA.write(f"**{h}**")
            if cB.button("❌", key=f"dhsn_{h}"): hsn_codes.remove(h); settings["hsn_codes"] = hsn_codes; save_settings_to_sheet(settings); st.rerun()
        st.write("---"); n_hsn = st.text_input("New HSN:")
